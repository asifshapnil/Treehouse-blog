...
$adsql = "SELECT * FROM admin WHERE uid = '$session'";
...
$admin = $adrow['uid'];
  ...
  if ($session == true) {
    $notadmin = "<div class='container'><div class='error'><h3>You are signed in</h3> </div></div>";
    function redirect($url){
         if (!headers_sent())
         {
             header('Location: '.$url);
             ...
             $url = "adminprofile.php?notadmin=$notadmin";
         redirect($url);
  }
